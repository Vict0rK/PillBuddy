...
| LOW YI SAN                              |  - Developed and implemented the web dashboard using React (frontend) and Flask (backend). - Integrated MongoDB Atlas for data storage and management.  - Created APIs for medication box setup, logs, weekly adherence graph, and statistics. - Designed and implemented user interface features for caregivers to manage medication schedules and monitor adherence.     |
